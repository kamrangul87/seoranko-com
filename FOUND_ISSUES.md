...
| Feature freeze vs public tool | `fdb7c12` + `20260910080000` | Public Index Diagnosis + `public_scans` shipped 10 Sept under an explicit “ship funnel now” Cursor prompt while beta reliability freeze still applied — process miss. Documented in `docs/PHASE5_AGENTIC_FIX_RUN.md`; tool on hold; deny policies added. |
| `src/app/api/cron/gsc-sync/route.ts` | inspection catch | URL Inspection batch errors are logged and swallowed; cron still returns `ok: true` for metrics. No email/in-app alert when `last_sync_at` goes stale — explains silent multi-day `url_metrics_daily` lag. |
| `src/lib/gsc/inspection-scheduler.ts` | empty queue | Candidate selection runs before `reserveQuota`; empty batch returned with `stoppedReason=empty` and no `last_error` — matches prod `quota_usage=0` + clean cron. Mitigated: last_error now written on empty/zero-reserve/throw. |
