/**
 * Owner-facing findings copy — static per verdict code.
 * NOT model-generated. NOT stored per finding. Resolved at read time.
 * Plain-English lines must not claim a ranking effect.
 */

import type { SourceTier } from './types'

/** One sentence for site owners — listable verdicts (actionable + informational). */
export const OWNER_PLAIN_ENGLISH: Record<string, string> = {
...
/** Why SEORANKO did not fix / left alone — suppress, route, skip, ok. */
export const WHY_NOT_FIXED: Record<string, string> = {
...
/** Primary rule tier by topic id. */
export const SOURCE_TIER_BY_TOPIC: Record<string, SourceTier> = {
  '1': 'VENDOR-DOCUMENTED',
  '3': 'VENDOR-DOCUMENTED',
  '4': 'VENDOR-DOCUMENTED',
  '5': 'STANDARD',
  ...
  '8': 'VENDOR-DOCUMENTED',
  '9': 'VENDOR-DOCUMENTED',
  '10': 'VENDOR-DOCUMENTED',
  '11': 'VENDOR-DOCUMENTED',
  '12': 'VENDOR-DOCUMENTED',
  '13': 'STANDARD',
  '14': 'VENDOR-DOCUMENTED',
  '15': 'VENDOR-DOCUMENTED',
  '16': 'VENDOR-DOCUMENTED',
  '17': 'VENDOR-DOCUMENTED',
  '19': 'VENDOR-DOCUMENTED',
  '20': 'VENDOR-DOCUMENTED',
  '21': 'VENDOR-DOCUMENTED',
  '22': 'STANDARD',
  '24': 'STANDARD',
  '25': 'STANDARD',
  '26': 'VENDOR-DOCUMENTED',
  '27': 'VENDOR-DOCUMENTED',
  '28': 'STANDARD',
  '29': 'STANDARD',
  '2b': 'VENDOR-DOCUMENTED',
  '30': 'STANDARD',
  '31': 'STANDARD',
  '33': 'VENDOR-DOCUMENTED',
  '34': 'STANDARD',
  '35': 'VENDOR-DOCUMENTED',
  '36': 'VENDOR-DOCUMENTED',
  '37': 'VENDOR-DOCUMENTED',
  '38': 'VENDOR-DOCUMENTED',
  '39': 'VENDOR-DOCUMENTED',
  '42': 'VENDOR-DOCUMENTED',
  '43': 'VENDOR-DOCUMENTED',
  '45': 'OBSERVED',
  '46': 'VENDOR-DOCUMENTED',
  '47': 'STANDARD',
...
export const PRIMARY_SOURCE_ID_BY_TOPIC: Record<string, number> = {
  '1': 4,
  '3': 30,
  '4': 36,
  '5': 36,
...
export function ownerPlainEnglish(verdict: string): string | null {
  return OWNER_PLAIN_ENGLISH[verdict] ?? null
}

export function whyNotFixed(verdict: string): string | null {
  return WHY_NOT_FIXED[verdict] ?? null
}

/** Fallback when a suppress/route/skip/ok code lacks a dedicated sentence. */
export function whyNotFixedOrFallback(verdict: string): string {
  const mapped = whyNotFixed(verdict)
  if (mapped) return mapped
  if (verdict.startsWith('route-')) {
    return 'Routed to another topic — handle it there, not under this detector.'
  }
  if (verdict.startsWith('suppress-')) {
    return 'SEORANKO checked this and left it alone.'
  }
  if (verdict.startsWith('skip-')) {
    return 'Skipped — out of scope for this check.'
  }
  if (verdict === 'ok' || verdict.startsWith('ok-') || verdict.startsWith('ok/')) {
    return 'Check passed — nothing to fix.'
  }
  return 'SEORANKO checked this and left it alone.'
}

export function sourceTierForTopic(topicId: string): SourceTier {
  return SOURCE_TIER_BY_TOPIC[topicId] ?? 'SEORANKO PRODUCT DECISION'
}

export function primarySourceIdForTopic(topicId: string): number | null {
  return PRIMARY_SOURCE_ID_BY_TOPIC[topicId] ?? null
}
...
/** Why this listable finding was not auto-applied (surface-class reason). */
export function whyFindingNotAutoFixed(opts: {
  autoFixable: boolean
  reportOnly: boolean
  surfaceClass: string
}): string | null {
  ...
  if (opts.surfaceClass === 'informational' || opts.reportOnly) {
    return 'SEORANKO reports this for awareness and will not auto-apply a change.'
  }
  if (opts.surfaceClass === 'finding' || opts.surfaceClass === 'report-only') {
    return 'No safe mechanical fix is available for this verdict.'
  }
  return 'SEORANKO did not auto-apply a change for this finding.'
}
